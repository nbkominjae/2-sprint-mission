...
SELECT pizza_id , SUM(quantity) AS total_order_quantities
FROM order_details
GROUP BY pizza_id
;
...
SELECT pizzas.id, SUM(quantity * pizzas.price) AS total_revenue
FROM order_details
...
SELECT orders.date AS order_date, COUNT(order_id) AS order_count , SUM(quantity) AS total_quantity 
FROM order_details
...
FROM order_details
	...
	JOIN orders ON orders.id = order_details.order_id
WHERE orders.id = 78
GROUP BY pizza_types.name, pizzas.size, pizzas.price, pizza_price, order_details.quantity
ORDER BY pizzas.size , pizzas.price DESC
;
...
SELECT pizzas.size , SUM(pizzas.price * quantity) AS total_money
FROM order_details
	JOIN pizzas ON pizzas.id = order_details.pizza_id
GROUP BY pizzas.size
ORDER BY SUM(pizzas.price * quantity) DESC
;
...
SELECT pizza_types.name, SUM(order_details.quantity * pizzas.price) AS total_money
FROM order_details
	JOIN pizzas ON pizzas.id = order_details.pizza_id
	JOIN pizza_types ON pizza_types.id = pizzas.type_id
